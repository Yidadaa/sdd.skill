...
- **No large inline types in params/props** — if an inline object type has 3+ fields, extract it as a named type or interface.
- **Prefer inference over redefinition** — use `Infer`, `Pick`, `Omit`, `ReturnType`, `Parameters`, `ComponentProps`, etc. instead of re-declaring types that can be derived from an existing source of truth (DB schema, tRPC router, Zod schema, etc.).
- **No `as any`, `@ts-ignore`, `@ts-expect-error`** — fix types properly.
- **No `as never` or `as unknown as T`** — find the correct type.
- **No `Record<string, any>` or overly broad types** — define or derive specific types.
...
- **No hard-coded route strings** — import from `constant/routes.ts`.
- **No hard-coded model/tier/group strings** — use enum values or constants from the relevant module.

#### General Code Style (project-specific)
- **No inline styles** — use Tailwind classes.
- **No empty catch blocks** — handle or log the error.
- **No console.log in production code** — use proper logging.
- **No barrel `index.ts` re-exports** — import directly.
- **No TanStack server functions** — use tRPC.
